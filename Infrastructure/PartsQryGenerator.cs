...
namespace Infrastructure
{
    public class PartsQryGenerator<TEntity> : IPartsQryGenerator<TEntity> where TEntity : class
    {
        ...
        private string _characterParameter;
        ...
        public PartsQryGenerator(char characterParameter = '@')
        {
            ...
            _propertiesNames = _properties.Where(a => !IsComplexType(a)).Select(a => a.Name).ToArray();
            _typeName = type.Name;
        }
        ...
        public string GenerateInsert(string identityField = null)
        {
            ...
            string[] parametersCampsCol = propertiesNamesDef.Select(a => $"{_characterParameter}{a}").ToArray();
        ...
        public string GenerateSelect()
        {
            ...
            string selectPart = string.Join(separator, _propertiesNames);

            sb.AppendLine(selectPart);

            string fromPart = $"FROM {_typeName}";
        ...
        private object GenerateWhere(object parameters)
        {
            ParameterValidator.ValidateObject(parameters, nameof(parameters));

            var filtersPksFields = parameters.GetType().GetProperties().Select(a => a.Name).ToArray();

            if (!filtersPksFields?.Any() ?? true)
                throw new ArgumentException($"El parameter filtersPks isn't valid. This parameter must be a class type", nameof(parameters));
